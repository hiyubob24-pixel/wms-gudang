<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('stock-out.create') }}" class="inline-flex w-full items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm dark:shadow-none transition hover:bg-blue-700 sm:w-auto">
                + Input Barang Keluar
            </a>
        </div>
    </x-slot>
                        ...
                        @forelse($stockOuts as $item)
                            <article class="rounded-xl border border-gray-200 dark:border-slate-700 p-4 shadow-sm dark:shadow-none">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <span class="inline-flex rounded-full bg-red-100 dark:bg-rose-900/30 px-2.5 py-1 text-xs font-semibold text-red-800">-{{ $item->quantity }}</span>
                                </div>

                                <dl class="mt-4 space-y-2 text-sm">
                                    <div class="flex items-start justify-between gap-4">
                                        <dt class="text-gray-500 dark:text-slate-400">Dari Rak</dt>
                                ...
                                <div class="mt-4 border-t border-gray-100 dark:border-slate-700/60 pt-3 text-sm font-medium">
                                    @if(auth()->user()->role === 'admin')
                                        <a href="{{ route('stock-out.edit', $item->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                    @else
                        ...
                        <table class="min-w-full bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700">
                            <thead class="bg-gray-50 dark:bg-slate-800 border-b">
                                <tr>
                                    ...
                                    <th class="py-3 px-6 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">Dari Rak</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                                @forelse($stockOuts as $item)
                                    <tr class="hover:bg-gray-50 dark:bg-slate-800 transition duration-150">
                                        ...
                                        <td class="py-4 px-6 text-center whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 dark:bg-rose-900/30 text-red-800">
                                                -{{ $item->quantity }}
                                            </span>
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-6 text-center text-sm text-gray-500 dark:text-slate-400">Belum ada riwayat barang keluar.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
