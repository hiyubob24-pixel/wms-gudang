            ...
            <div class="app-form-card p-6 sm:p-8">
                <form action="{{ route('stock-out.update', $stockOut->id) }}" method="POST">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            <label class="app-field-label" for="rak_id">Lokasi Rak</label>
                            <div class="app-field-select-wrap">
                                <select name="rak_id" id="rak_id" class="app-field-select">
                                    @foreach($raks as $rak)
                                        <option value="{{ $rak->id }}" {{ old('rak_id', $stockOut->rak_id) == $rak->id ? 'selected' : '' }}>
                                            {{ $rak->display_name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="date_time" id="date_time"
                                value="{{ old('date_time', date('Y-m-d\TH:i', strtotime($stockOut->date_time))) }}"
                                class="app-field-input">
                        </div>
                    </div>
